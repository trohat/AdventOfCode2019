console.log("AOC 2018 - Day 10: Monitoring Station");
...
const getAngle = (x,y) => {
  ...
  if (angle <= 90) angle = 90 - angle; else angle = 450 - angle;
  return angle;
};
...
const task2 = ( set, stationX, stationY, position) => {
  ...
  set.forEach(asteroid => {
    ...
    const xDist = asteroidX - stationX;
    const yDist = asteroidY - stationY;
    let angle = getAngle(xDist, yDist);
    angles.add(angle);
    let distance = countPythagoras(xDist, yDist);
    if ((!(angle in anglesAndAsteroids)) || anglesAndAsteroids[angle].distance > distance) anglesAndAsteroids[angle] = {
      asteroidX, asteroidY, distance
    }
  })
...
const [ result, sX, sY ] = task1(inputdata);

console.log("Task1: " + result);

console.log(task2(inputdata, sX, sY, 200));

runTask2 = position => {
  console.log(task2(testdata, sX, sY, position))
}
