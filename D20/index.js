...
const prepare = (data) => {
    ...
    for (const line of data) {
        ...
        for (const char of line) {
            ...
            maze[i].push({ char, isWall, isPortal, isPassage, portalized: null, portal: null, distance: null, visited: false, pathHere: false })
            j++;
        }
        maze[i].push({ char: " "});
        i++;
    ...
    while (fieldsToSearch.length > 0) {
        ...
        if (actualField.portal && !actualField.portal.beenThrough) {
            ...
            if (actualField.portal.portalName == "ZZ") continue;
